...
export default function SalaryPage() {
  ...
  // Reload after an in-page Skatteverket reconnect (the BankID popup posts
  // this message and closes). The OAuth callback awaits the skattekonto sync
  // + AGI auto-settlement before responding, so this refetch already sees
  // fresh tax-payment state instead of racing a background job.
  useEffect(() => {
    function handleMessage(event: MessageEvent) {
      if (event.origin !== window.location.origin) return
      if (event.data?.type === 'skatteverket-oauth-success') load()
    }
