...
export function validateStatementIntegrity(
  ...
  if (report.previous_period) {
    const previousAssets = report.balansrakning.total_assets_previous
    const previousEquityLiabilities =
      report.balansrakning.total_equity_liabilities_previous
    if (
      previousAssets === null ||
      previousEquityLiabilities === null ||
      Math.round(previousAssets * 100) !== Math.round(previousEquityLiabilities * 100)
    ) {
      push(
        issues,
        'AR-COMPARATIVE-BALANCE-MISMATCH',
        'error',
        'statements',
        'Jämförelseårets balansräkning balanserar inte i årsredovisningen.',
      )
    }
  }
  ...
  if (report.previous_period) {
    const comparativeIncomeStatementResult = report.resultatrakning.find(
      (row) => row.label === 'Årets resultat' && row.previous !== null,
    )?.previous ?? undefined
    const comparativeBalanceSheetResult = report.balansrakning.equity_liabilities.find(
      (row) => row.label === 'Årets resultat' && row.previous !== null,
    )?.previous ?? undefined
    if (
      comparativeIncomeStatementResult === undefined ||
      comparativeBalanceSheetResult === undefined
    ) {
      push(
        issues,
        'AR-COMPARATIVE-RESULT-MISSING',
        'error',
        'statements',
        'Jämförelseåret saknar raden Årets resultat i resultat- eller balansräkningen.',
      )
    } else if (
      Math.round(comparativeIncomeStatementResult * 100) !==
      Math.round(comparativeBalanceSheetResult * 100)
    ) {
      push(
        issues,
        'AR-COMPARATIVE-RESULT-MISMATCH',
        'error',
        'statements',
        'Jämförelseårets resultat stämmer inte mellan resultat- och balansräkningen.',
        'Kontrollera att jämförelseåret bygger på ett slutligt bokslut.',
      )
    }
  }
