...
function model(signedAt: string | null): CanonicalAnnualReport {
  const mapping = mapTrialBalancesToK2(
    { full: CURRENT_FULL, preClosing: CURRENT_PRE_CLOSING },
    null,
  )
...
describe('annualReportContentHash', () => {
  ...
  it('refuses matching PDF and iXBRL comparatives from an unfinished prior year', async () => {
    const rpc = vi.fn()
    const inconsistent = model(null)
    const previousFull = [
      { account_number: '1930', account_name: 'Bank', closing_debit: 90, closing_credit: 0 },
      { account_number: '2081', account_name: 'Share capital', closing_debit: 0, closing_credit: 80 },
      { account_number: '3010', account_name: 'Revenue', closing_debit: 0, closing_credit: 10 },
    ]
    const mapping = mapTrialBalancesToK2(
      { full: CURRENT_FULL, preClosing: CURRENT_PRE_CLOSING },
      { full: previousFull, preClosing: previousFull },
    )
    const balanceRows = buildBrRows(mapping)
    inconsistent.report.previous_period = {
      name: '2024',
      period_start: '2024-01-01',
      period_end: '2024-12-31',
    }
    inconsistent.report.resultatrakning = buildRrRows(mapping)
    inconsistent.report.balansrakning = {
      assets: balanceRows.assets,
      equity_liabilities: balanceRows.equityLiabilities,
