-- bulk_book_transactions: refuse a mixed-currency selection.
--
-- Why this belongs in SQL and not only in the route:
--   `GRANT EXECUTE ... TO authenticated` means any signed-in session can
--   call this RPC straight over PostgREST, and the pending-operations
--   executor (lib/pending-operations/commit.ts) passes tx_ids through
--   without inspecting currency. Both reach the loop below, which sums
--   v_tx.amount across rows regardless of v_tx.currency: 100 EUR + 100 SEK
--   became the scalar 200, and the resulting samlingsverifikat stated an
--   amount matching no affärshändelse.
--
-- Legal basis: BFL 4 kap 6 § requires the bokföring to be presented in one
-- and the same redovisningsvaluta (SEK, or EUR for the entities allowed to
-- elect it). Combined with BFL 5 kap 7 § ("belopp"), a verifikat spanning
-- two currencies is not representable without per-transaction FX rates.
-- Cross-currency batches belong in the FX-aware batch-allocate flow, which
-- books the kursdifferens on 7960/3960.
--
-- NULL currency is legacy for the column default 'SEK' (transactions.currency
-- is `text default 'SEK'`, nullable), and the application reads NULL as SEK
