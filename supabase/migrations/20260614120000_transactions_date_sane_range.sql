...
--
-- The manual "add transaction" form inserts straight into `transactions` via
-- the browser Supabase client (no server route, no server-side validation).
-- A native <input type="date"> whose year subfield is over-typed can emit a
-- 6-digit year (e.g. '202403-02-05' = year 202403); Postgres' `date` type
-- accepts it, the row saves, and every date-fns formatter then throws
-- RangeError on render — taking down the whole dashboard route via the error
-- boundary. (Real incident, 2026-06: company 4e4e41e7 locked out of the
-- transactions page.)
--
