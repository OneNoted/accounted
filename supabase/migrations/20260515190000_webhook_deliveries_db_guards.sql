...
-- never bypass: (1) BFNAR 2013:2 kap 8 § immutability extends to DELETE on
...
BEGIN
  IF OLD.status IN ('delivered', 'dead') THEN
    RAISE EXCEPTION
      'webhook_deliveries row in terminal status (%) cannot be deleted (BFNAR 2013:2 kap 8 § retention)',
      OLD.status
