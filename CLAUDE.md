...
- **Auth**: Supabase email+password + TOTP MFA, enforced **application-side**, not in RLS. `REQUIRE_MFA=true` enables server authorization in both hosted and self-hosted deployments; set `NEXT_PUBLIC_REQUIRE_MFA=true` as well so client security UI reflects the same policy. API routes wrap `withRouteContext`: it is the only path that enforces MFA, so never hand-roll `supabase.auth.getUser()` in a route.
