...
/**
 * The agent's competence (domain-knowledge atoms) and top learned facts,
 * shown read-only on the "Vad din agent vet" overview. The full, editable
 * management lives in /settings/assistant; each section links there.
 */

const TIER_ORDER: AtomTier[] = ['horizontal', 'vertical', 'modifier']

export async function AgentCompetenceSections({ competence }: { competence: AgentCompetence }) {
  const t = await getTranslations('agentKnowledge')
  const { atoms, facts, factsActiveTotal } = competence
  ...
  return (
    <div className="grid gap-4 lg:grid-cols-2">
      {/* Kompetens: the domain knowledge it ships with */}
      <Card>
        <CardHeader>
      ...
      {/* Fakta: what it has learned/remembers about this company */}
      <Card>
        <CardHeader>
